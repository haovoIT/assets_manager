...
class PdfDSThanhLyApi {
  static Future<File> generate(
      List<ThanhLy> list, String email, String name) async {
    ...
    return PdfApi.saveDocument(name: "Danh_Sach_Thanh_Ly.pdf", pdf: pdf);
  }
  ...
  static pw.Widget buildTitle() => pw.Column(
        ...
        children: [
          ...
          pw.Text(
            'Danh Sách Thông Tin Thanh Lý Tài sản',
  ...
  static pw.Widget buildTable(List<ThanhLy> list, int index) {
    index = 0;
    final headers = [
      'STT',
      'Tên Tài Sản',
    ...
    final data = list.map((item) {
      return [
        ...
        item.Nguyen_gia_TL,
        item.Ngay_TL != null
            ? DateFormat('dd/MM/yyyy').format(DateTime.parse(item.Ngay_TL!))
            : ""
      ];
    }).toList();
  ...
  static pw.Widget buildFooter(String email) => pw.Column(
        ...
        children: [
          ...
          pw.Divider(),
          pw.Text("Danh Sách Thông Tin Thanh Lý - Email: " + email,
              style: pw.TextStyle(fontStyle: pw.FontStyle.italic)),
