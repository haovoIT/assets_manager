...
class PdfDSHopDongApi {
  static Future<File> generate(List<Contract> list,String email,String name) async {
  ...
  static pw.Widget buildTable(List<Contract> list,int index) {
    final headers = [
      'STT',
      'Số HD',
      'Tên HD',
    ...
    final data = list.map((item) {
      return [
        ...
        item.SoHD,
        item.TenHD,
        item.NgayKy!= null? DateFormat('dd/MM/yyyy').format(DateTime.parse(item.NgayKy!)):"",
        item.NHH!= null ? DateFormat('dd/MM/yyyy').format(DateTime.parse(item.NHH!)):"",
        item.NCC,
        item.ND
      ];
    }).toList();
  ...
  static pw.Widget buildFooter(String email) => pw.Column(
    ...
    children: [
      ...
      pw.Divider(),
      pw.Text("Danh Sách Hợp Đồng - Email: "+ email, style: pw.TextStyle(fontStyle: pw.FontStyle.italic)),
