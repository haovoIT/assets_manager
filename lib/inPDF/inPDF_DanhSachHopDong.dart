...
class PdfDSHopDongApi {
  static Future<File> generate(
      List<Contract> list, String email, String name) async {
    ...
    return PdfApi.saveDocument(name: "Danh_Sach_Hop_Dong.pdf", pdf: pdf);
  }
  ...
  static pw.Widget buildTable(List<Contract> list, int index) {
    ...
    final data = list.map((item) {
      return [
        ...
        item.numberContract,
        item.name,
        item.signingDate != null
            ? DateFormat('dd/MM/yyyy').format(DateTime.parse(item.signingDate!))
            : "",
  ...
  static pw.Widget buildFooter(String email) => pw.Column(
        ...
        children: [
          ...
          pw.Divider(),
          pw.Text("Danh Sách Hợp Đồng - Email: " + email,
              style: pw.TextStyle(fontStyle: pw.FontStyle.italic)),
